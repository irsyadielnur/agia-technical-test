  ...
  return (
    <Link href={`/catalog/${product.id}`} className="block h-full">
      <motion.div
        ...
        className="bg-white rounded-2xl border border-gray-150 overflow-hidden shadow-xs hover:shadow-xl hover:shadow-gray-200/50 transition-all duration-300 ease-in-out group flex flex-col h-full cursor-pointer relative"
      >
        ...
        <div className="relative aspect-square overflow-hidden bg-gray-50 border-b border-gray-100 shrink-0">
          ...
          {/* Category tag */}
          <span className="absolute top-3 left-3 inline-flex px-2.5 py-1 rounded-full text-[9px] font-bold uppercase tracking-wider bg-white/95 backdrop-blur-xs text-gray-800 shadow-xs border border-gray-250/50 z-2">
            {product.category}
          </span>
          {/* Stock Badge */}
          {product.stock <= 5 && (
