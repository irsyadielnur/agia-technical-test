    ...
    <>
      <nav className="flex justify-between items-center px-4 md:px-8 lg:px-20 w-full h-16 bg-brand-bg/50 backdrop-blur-md fixed top-0 z-50">
        ...
        <button
          ...
          className="md:hidden p-2 rounded-full hover:bg-gray-150 active:bg-gray-200 transition-colors cursor-pointer text-gray-700"
        >
          ...
          {NAV_LINKS.map((link) => {
            ...
            return (
              <Link
                key={link.href}
                href={link.href}
                className={`hover:font-semibold transition-all duration-100 ${isActive ? "font-semibold text-gray-900" : "font-normal"}`}
              >
                {link.label}
              </Link>
            );
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            {isDropdownOpen && (
              <div className="absolute right-0 mt-2 w-48 bg-brand-bg rounded-xl shadow-lg border border-gray-100 py-2 z-50">
                ...
                >
                  {theme === "dark" ? (
                    <Sun size={16} className="text-yellow-500 fill-yellow-500 animate-pulse" />
                  ) : (
                    <Moon size={16} />
                  )}
                  <span>{theme === "dark" ? "Mode Terang" : "Dark Mode"}</span>
                </button>
              </div>
            )}
          </div>
            ...
            {NAV_LINKS.map((link) => {
              ...
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`text-lg hover:font-semibold transition-all duration-100 ${isActive ? "font-semibold text-gray-900" : "font-normal text-gray-600"}`}
                >
                  {link.label}
                </Link>
              );
